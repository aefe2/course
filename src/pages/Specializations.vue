<template>
  ...
  <div class="btn-link-container">
    <my-button @click="$router.push('/add-specialization')" class="create-patient"><span>Добавить специальность</span>
    </my-button>
  </div>
  <div class="table-wrapper">
    <table class="table">
      ...
      <tr v-for="specialization in specializations" :key="specialization.id">
        ...
        <td :class="theme"><a @click="deleteSpecializations(specialization.id)">X</a></td>
      </tr>
      </tbody>
    </table>
  </div>
...
export default {
  ...
  methods: {
    async deleteSpecializations(index) {
      try {
        const response = await axios.get('http://localhost/CodingOnSideOfServer/api/delete_specialization', {
          headers: {
    ...
    async fetchSpecializations() {
      try {
        const response = await axios.get('http://localhost/CodingOnSideOfServer/api/specializations')
